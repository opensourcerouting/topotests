...
#
# test_fabricd_folded_clos_06_unnumbered_single_tier0.py
...
"""
test_fabricd_folded_clos_06_unnumbered_single_tier0.py: Test for OpenFabric Folded Clos Topo.
...
topoJson = open('{0}/topo/unnumbered/clos_topo_single_tier0.json'.format(CWD)).read()
...
def test_check_NO_convergence():
    "Verifies OpenFabric Network does not converge by checking for Tier"
...
def test_add_tier0_and_check_convergence():
    "Verifies OpenFabric Network does converge after adding 2nd Tier0"
    ...
    # Add tier 0 to router d4
    logger.info('Adding "fabric-tier 0" to d4 openfabric config')
    tgen.gears['d4'].vtysh_cmd('configure terminal\nrouter openfabric 1\nfabric-tier 0\nend', isjson=False)
