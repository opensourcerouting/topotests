...
topoJson = open("clos_topo_expanding.json").read()
...
def setup_module(mod):
    ...
    for routerN in topo['routers'].items():
        if topo['routers'][routerN[0]]['started'] == 1:
            listRouters.append(routerN[0])
    listRouters.sort()
...
def test_verify_ipv4_routing_table():
    ...
    for curRouter in listRouters:
        filename = '{0}/testdata/{1}_ipv4_routes_before_expand.json'.format(CWD, curRouter)
...
def test_verify_ipv6_routing_table():
    ...
    for curRouter in listRouters:
        filename = '{0}/testdata/{1}_ipv6_routes_before_expand.json'.format(CWD, curRouter)
        expected = json.loads(open(filename, 'r').read())
...
def test_add_expansion():
    ...
    logger.info('Configuring additional nodes')
...
def test_check_convergence_expanded():
    ...
    logger.info('Waiting for convergence of additional routers')

    tgen = get_topogen()
    timeout = 60
    while timeout > 0:
        logger.info('Waiting for convergence (timeout {}s)'.format(timeout))
        database = tgen.gears['f2'].vtysh_cmd('show openfabric database detail', isjson=False)
        m = re.search('Tier: undefined', database, re.DOTALL)
        if m:
            # Still have undefined Tier levels in topology
            sleep(4)
...
#         with open('/tmp/{0}_ipv4_routes_after_expand.json'.format(curRouter), 'w') as route_file:
...
#         with open('/tmp/{0}_ipv6_routes_after_expand.json'.format(curRouter), 'w') as route_file:
#             #json.dump(allroutes, route_file)
#             route_file.write(json.dumps(allroutes, indent=2))
...
def test_verify_ipv4_routing_table_after_expand():
    ...
    for curRouter in listRouters:
        filename = '{0}/testdata/{1}_ipv4_routes_after_expand.json'.format(CWD, curRouter)
        expected = json.loads(open(filename, 'r').read())
        actual = tgen.gears[curRouter].vtysh_cmd('show ip route json', isjson=True)
...
def test_verify_ipv6_routing_table_after_expand():
    ...
    for curRouter in listRouters:
        filename = '{0}/testdata/{1}_ipv6_routes_after_expand.json'.format(CWD, curRouter)
        expected = json.loads(open(filename, 'r').read())
        actual = tgen.gears[curRouter].vtysh_cmd('show ipv6 route json', isjson=True)
