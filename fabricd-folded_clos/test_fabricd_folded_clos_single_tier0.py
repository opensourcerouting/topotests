...
topoJson = open("clos_topo_single_tier0.json").read()
...
def test_check_NO_convergence():
    ...
    logger.info('Waiting for convergence (SHOULD NOT CONVERGE)')

    tgen = get_topogen()
    timeout = 120
    while timeout > 0:
        logger.info('Waiting for convergence - (SHOULD NOT CONVERGE) (timeout {}s)'.format(timeout))
