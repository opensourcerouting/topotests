...
#
# test_fabricd_folded_clos_04_numbered_expanding.py
...
"""
test_fabricd_folded_clos_04_numbered_expanding.py: Test for OpenFabric Folded Clos Topo.
...
def test_check_convergence_expanded():
    ...
    if not wait_fabric_convergence(tgen, 'f2'):
...
def test_verify_ipv4_routing_table_after_expand():
    ...
    errors = verify_routing_table(tgen, topo, 'ipv4', filename)
...
def test_verify_ipv6_routing_table_after_expand():
    ...
    filename = '{0}/testdata/'.format(CWD)+'{0}_{1}_routes_after_expand.json'
    errors = verify_routing_table(tgen, topo, 'ipv6', filename)
